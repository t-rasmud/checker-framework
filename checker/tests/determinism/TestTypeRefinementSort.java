...
public class TestTypeRefinementSort {
    ...
    void testSort2(
            ...
            @Det int index) {
        ondetList.sort(cmp);
        System.out.println(ondetList.get(index));
    }
    ...
    //    // TODO: Uncomment this when @ThisDet is implemented and Integer implements
    // Comparable<@ThisDet
    //    // Integer> rather than Comparable<@Det Integer>.

    //    void testSort12(@PolyDet List<@PolyDet("use") Integer> polyList) {
    //        Collections.sort(polyList);
    //        // ::error: assignment.type.incompatible
    //        @PolyDet("down") List<@Det Integer> tmp = polyList;
    //    }
    ...
    <T extends @Det Object> void sortGeneric3(
            @OrderNonDet List<T> list, @Det Comparator<T> cmp, @Det int index) {
        list.sort(cmp);
        @Det T result = list.get(index);
    }
}
