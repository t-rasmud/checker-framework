...
public final class Chars {
     ...
     *     {@link Character#MAX_VALUE} if it is too large, or {@link Character#MIN_VALUE} if it is
     *     too small
     */
     ...
     * Arrays.copyOfRange(array, i, i + target.length)} contains exactly the same elements as {@code
     * target}.
     *
    ...
    /**
     * Returns the value nearest to {@code value} which is within the closed range {@code
     * [min..max]}.
     *
     ...
     * specified minimum length. If {@code array} already has a length of at least {@code
     * minLength}, it is returned directly. Otherwise, a new array of size {@code minLength +
     * padding} is returned, containing the values of {@code array}, and zeroes in the remaining
     * places.
     *
     ...
     * href="http://en.wikipedia.org/wiki/Lexicographical_order">lexicographically</a>; not
     * advisable for sorting user-visible strings as the ordering may not match the conventions of
     * the user's locale. That is, it compares, using {@link #compare(char, char)}), the first pair
     * of values that follow any common prefix, or when one array is a prefix of the other, treats
     * the shorter array as the lesser. For example, {@code [] < ['a'] < ['a', 'b'] < ['b']}.
     *
    ...
    /**
     * Reverses the elements of {@code array} between {@code fromIndex} inclusive and {@code
     ...
     *
     * <p>The returned list maintains the values, but not the identities, of {@code Character}
     * objects written to or read from it. For example, whether {@code list.get(0) == list.get(0)}
