...
public class CFGTranslationPhaseThree {
    ...
    public static ControlFlowGraph process(ControlFlowGraph cfg) {
        ...
        for (Block c : worklist) {
            @SuppressWarnings("determinism") // process is order insensitive: removing predecessors
            @Det BlockImpl cur = (BlockImpl) c;
            for (Block tmpPred : new HashSet<>(cur.getPredecessors())) {
                @SuppressWarnings(
                        "determinism") // process is order insensitive: removing predecessors
                @Det Block pred = tmpPred;
        ...
        // remove empty blocks
        for (Block tmpCur : worklist) {
            @SuppressWarnings("determinism") // process is order insensitive: removing empty blocks
            @Det Block cur = tmpCur;
        ...
        switch (pred.getType()) {
            ...
            case EXCEPTION_BLOCK:
                ...
                if (e.getSuccessor() == cur) {
                    ...
                    Set<Map.@Det Entry<TypeMirror, Set<Block>>> entrySet =
