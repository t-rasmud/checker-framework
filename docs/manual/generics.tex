...
possible to enable it on all classes by default with names matching a regular
expression. The \<-AdefaultHasQualifierParameter> command-line option takes a
semicolon separated list of pairs containing an annotation and a regular
expression.

Each pair should be of the form ``\<annotation:pattern>'' where \<annotation> is
the fully qualified name of the top annotation in the hierarchy to enable
\<@HasQualifierParameter> for and \<pattern> is a regular expression. This
option is the same as writing ``\<@HasQualifierParameter(annotation.class)>'' on
all classes that match \<pattern>. The \<pattern> should not contain a
semicolon.

For example, adding this command-line option enables \<@HasQualifierParameter>
for all classes in \code{mypackage}.

\begin{Verbatim}
-AdefaultHasQualifierParameter=org.checkerframework.checker.tainting.qual.Tainted:^mypackage\..*>
\end{Verbatim}

When using \<-AdefaultHasQualifierParameter>, it's possible to disable
