...
\item \ahrefloc{nonempty-checker}{NonEmpty Checker} to ensure that a Collection, Map, or an Iterator contains atleast one element. (see \chapterpageref{nonempty-checker})
\item
  \ahrefloc{iteration-checker}{Iteration Checker} to ensure that \code{Iterator.next()} does not
  throw NoSuchElementException (see \chapterpageref{iteration-checker})
\item
