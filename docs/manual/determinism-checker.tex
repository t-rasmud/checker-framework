...
explicitly annotated with \code{@NonDet} or \code{@PolyDet}, and the component type is
unannotated, then then the component type and all unannotated nested component
types are defaulted to the same type as the outer collection type. Otherwise, the explicitly written type or
...
\begin{Verbatim}
    ...
    @PolyDet List<String> list3;
\end{Verbatim}
the type of \code{list1} is \code{@NonDet List<@NonDet String>}, the type of
\code{list2} is \code{@Det List<@Det String>}, and the type of \code{list3} is
\code{@PolyDet List<@PolyDet String>}. Notice, in both instances the
