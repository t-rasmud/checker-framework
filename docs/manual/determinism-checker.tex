...
\item[\refqualclass{checker/determinism/qual}{OrderNonDet}] indicates that
  a collection (that's any class whose declaration is annotated with checker/determinism/qual}{CollectionType},
   see Section~\ref{determinism-declaration-annotations}) or an array will have the same elements in every execution, but in a
...
\begin{description}
\item[\requalclass{checker/determinism/qual}{CollectionType}] is a type declaration annotation used
  to indicate that the class (and all of its subclasses) implements a collection.
\end{description}
