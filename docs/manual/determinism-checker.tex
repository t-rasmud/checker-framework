...
\item
  ...
  \<@Det>.
\item
  The default for fields is \<@PolyDet>. Thus, classes with fields will require
  \<@HasQualifierParameter> by default.
\end{itemize}
...
Section~\ref{determinism-hash-set}). The Determinism Checker instead makes the
default for local collections \code{@PolyDet}. Also, if a local collection is
unannotated, or annotated with \code{@PolyDet}, then any unannotated component
types are defaulted to \code{@PolyDet} as well. Otherwise it gets the regular
\code{@Det} default. The \code{@PolyDet} default lines up with the default for
method parameters and returns.
This allows the user to spend less time
...
\code{@PolyDet List<@PolyDet String>} and the type of \code{arr} is \code{@PolyDet
String @PolyDet []}, which are valid types. In the following declarations
\begin{Verbatim}
    @Det List<String> list1;
    @PolyDet List<String> list2;
\end{Verbatim}
the type of \code{list1} is \code{@Det List<@Det String>}, and the type of
\code{list2} is \code{@PolyDet List<@PolyDet String>}
Notice, in both instances the
