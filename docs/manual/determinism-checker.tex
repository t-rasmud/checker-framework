...
we cannot guarantee that the same element will be returned in every execution. For instance,
...
So we annotate the type of the array access expression (equivalently, the array element) as
...
the type of the array access expression is \<@NonDet>. Example:
\begin{Verbatim}
@Det int @Det [] arr;
@NonDet int index;
int val = arr[index];
...
Note that we change the type of an array access as described above if
the array access occurs on the rhs of an expression.
If we were to replace every array access with this rule, the checker would allow
...
Had we replaced the type of every \<@OrderNonDet> array access with
