...
any mutation of collections having types \code{@NonDet Collection<@OrderNonDet E>} or
\code{@NonDet Collection<@Det E>}. It achieves
this via a method declaration annotation \refqualclass{checker/determinism/qual}{CheckReceiverForMutation}.
...
\sectionAndLabel{Determinism of formatting routines that call \<toString>}{determinism-formatting-routines}
The \<toString> method in JDK's \<Object> class returns a string consisting of the hashCode of the receiver
on which \<toString> is called. Therefore we annotate it as
\code{public @NonDet String toString(@PolyDet Object this)}.

Formatting routines such as \<format> print the output of \<toString> method called on its arguments.
Annotating the arguments of \<format> as \<@Det> doesn't guarantee that it will print deterministic output
because the arguments' class may not have a \<toString> implementation that is deterministic.
To enforce the check that the argument types of a method have a deterministic \<toString> implementation,
the Determinism Checker defines a method declaration annotation
\refqualclass{checker/determinism/qual}{@RequiresDetToString}. When this annotation is written without
any annotation value on a method, every parameter of that method may only be passed arguments such
that their class's \<toString> method returns a \<@Det> or \<@PolyDet> result.
When one or more integers are specified as annotation values, only the parameters at those indices
ought to obey this restriction.
