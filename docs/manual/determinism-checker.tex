...
annotation is given the same instantiation as all of the
...
to be same in this hierarchy. Similarly, \<@PolyDet("noOrderNonDet")> and \<@PolyDet("useNoOrderNonDet")> are considered
to be the same.
At method call sites, \<@PolyDet("use")> gets replaced by the type qualifier that \<@PolyDet>
resolves to.
\<@PolyDet("useNoOrderNonDet")> gets replaced by the type qualifier that \<@PolyDet("noOrderNonDet")>
resolves to.
